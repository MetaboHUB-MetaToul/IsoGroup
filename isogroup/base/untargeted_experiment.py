...
class UntargetedExperiment(Experiment):
    ...
    def __init__(self, dataset:pd.DataFrame, tracer:str, ppm_tol:float, rt_tol:float, max_atoms:int = None, keep:str=None, sample_name = None, sample_type=None) : #  keep_best_candidate: bool = False, #  keep_richest: bool = False,
        """
    ...
    def run_untargeted_pipeline(self, enhance=None):
        ...
        if enhance:
           self.enhancer(clusters_df=self.all_clusters_df, sample_name=self.sample_name, sample_type=self.sample_type)
    ...
    def enhancer(self, clusters_df, sample_name, sample_type):
        """
        Improve the annotation of the file following execution in untargeted mode, 
        depending on the sample and its type.
        ...
        :param sample_type: Type of the sample (unlabeled, labeled etc.)
        """
        if sample_type == 'unlabeled':
            df_feature_found = unlabeled_enhancer.annotate_feature_found(clusters_df, sample_name, sample_type)
